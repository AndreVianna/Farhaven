...
func test_registry_has_small_tree() -> void:
	assert_bool(PropRegistry.has_def(&"00001")).is_true()


func test_registry_has_wood() -> void:
	assert_bool(PropRegistry.has_def(&"00010")).is_true()
...
func test_get_def_unknown_returns_null() -> void:
	var def = PropRegistry.get_def(&"99999")
...
func test_get_yield_type_returns_self_when_no_yield() -> void:
	...
	var yield_id: StringName = PropRegistry.get_yield_type(&"00010")
	assert_str(String(yield_id)).is_equal("00010")


func test_get_yield_type_returns_mapped_type_for_source() -> void:
	# Small Tree (00001) yields Wood (00010).
	var yield_id: StringName = PropRegistry.get_yield_type(&"00001")
	assert_str(String(yield_id)).is_equal("00010")


func test_get_yield_type_unknown_returns_same_type() -> void:
	var yield_id: StringName = PropRegistry.get_yield_type(&"99999")
	assert_str(String(yield_id)).is_equal("99999")


func test_get_tool_speed_default_is_one() -> void:
	var speed: float = PropRegistry.get_tool_speed(&"00001", &"fists")
...
func test_get_tool_speed_returns_configured_value() -> void:
	...
	var def = PropRegistry.get_def(&"00001")
	if def != null and def.tool_speed.has(&"axe"):
		var speed: float = PropRegistry.get_tool_speed(&"00001", &"axe")
...
func test_get_tool_speed_unknown_prop_returns_one() -> void:
	var speed: float = PropRegistry.get_tool_speed(&"99999", &"axe")
	assert_float(speed).is_equal_approx(1.0, 0.001)
