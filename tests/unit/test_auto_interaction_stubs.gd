...
func before_test() -> void:
	...
	_grid.prop_respawned.connect(_on_respawned)
...
func _on_respawned(coords: Vector2i, prop_type: StringName) -> void:
	_respawned_coords = coords
	_respawned_type = prop_type
	_respawned_count += 1


# --- Helpers ---

func _make_prop(type: StringName, tool_req: StringName = &"", remaining: int = 3, respawn: float = 0.0) -> Resource:
...
func test_respawn_ticks() -> void:
	var rn := _make_prop(&"P00010", &"", 0, 5.0)
	rn.max_amount = 3
...
func test_respawn_triggers_at_zero() -> void:
	var rn := _make_prop(&"P00010", &"", 0, 1.0)
	rn.max_amount = 3
	var tile := _make_tile(Vector2i.ZERO, [rn])
	_grid._tiles[Vector2i.ZERO] = tile
	...
	assert_str(_respawned_type).is_equal(&"P00010")
...
func test_respawn_time_zero_never_enters_queue() -> void:
	...
	var rn := _make_prop(&"P00010", &"", 1, 0.0)  # respawn_time = 0
	var tile := _make_tile(Vector2i.ZERO, [rn])
	_grid._tiles[Vector2i.ZERO] = tile
	_catalog._knowledge[&"wood_tree"] = _Catalog.KnowledgeState.CATALOGED

	# Position player at tile center so proximity check finds the prop
...
func test_respawn_queue_handles_multiple_entries() -> void:
	...
	assert_int(_respawned_count).is_equal(1)
	assert_str(_respawned_type).is_equal(&"P00013")
...
func test_pickup_collects_ground_items_by_proximity() -> void:
	...
	survival._ground_items[Vector2i.ZERO] = [
		{"item_type": &"P00010", "count": 3, "sub_hex": Vector2i.ZERO},
	]
	...
	assert_str(_pickup_name).is_equal(&"P00010")
	assert_int(_pickup_amount).is_equal(3)
	assert_int(_inv.get_count(&"P00010")).is_equal(3)
...
func test_pickup_multiple_items_by_proximity() -> void:
	var survival := FakeSurvivalSystem.new()
	survival._ground_items[Vector2i.ZERO] = [
		{"item_type": &"P00010", "count": 2, "sub_hex": Vector2i.ZERO},
		{"item_type": &"P00013", "count": 1, "sub_hex": Vector2i.ZERO},
	]
	survival.name = "SurvivalSystem"
	_player.add_child(survival)
	...
	assert_int(_inv.get_count(&"P00010")).is_equal(2)
	assert_int(_inv.get_count(&"P00013")).is_equal(1)
...
func test_pickup_emits_ground_item_picked_up() -> void:
	var survival := FakeSurvivalSystem.new()
	survival._ground_items[Vector2i.ZERO] = [
		{"item_type": &"P00020", "count": 4, "sub_hex": Vector2i.ZERO},
	]
	survival.name = "SurvivalSystem"
	_player.add_child(survival)
	...
	assert_int(_pickup_count).is_equal(1)
	assert_str(_pickup_name).is_equal(&"P00020")
...
func test_pickup_not_triggered_when_out_of_range() -> void:
	var survival := FakeSurvivalSystem.new()
	survival._ground_items[Vector2i.ZERO] = [
		{"item_type": &"P00010", "count": 3, "sub_hex": Vector2i(2, 0)},
	]
	survival.name = "SurvivalSystem"
	_player.add_child(survival)
