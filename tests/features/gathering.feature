Feature: Gathering and Discovery
  ...
  Scenario: Scan source unlocks its recipe
    Given a catalog with "P00001" in UNKNOWN state
    When the player scans prop "P00001" to completion
    Then catalog entry "P00001" is CATALOGED

  Scenario: Tool-gated gathering requires the right tool
    Given a recipe "R00007" that requires tool "pickaxe"
    When the player has no pickaxe equipped
    Then the recipe "R00007" condition check fails for has_tool

  Scenario: Gathering depletes source remaining count
    Given a prop "P00001" on tile 0,0 with remaining 5
  ...
  Scenario: Anomaly Fragment is scannable
    ...
    Then PropDef "P10001" has a catalogable capability
