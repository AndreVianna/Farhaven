...
**Status:** Draft v2 — gap-fix pass (Andre / 2026-04-25)
...
Farhaven life evolved on a world where the dominant body plan is **six-limbed**, not four. Where Earth radiated quadrupeds + birds + insects, Farhaven radiated hexapods + tetrawing aerials + brachiating mixed-limb forms. Reptile-grade lineages convergently evolved plumage instead of staying scaly, and stayed small. Megafauna are mammal-analogue (fur, warm-blooded). Most life on the planet lives somewhere between "rat" and "elephant×2" in size — there are no insects, no microfauna, and no kaiju. The current code (`scripts/fauna/fauna_manager.gd`) treats fauna as cheap dictionaries spawned at night and despawned at dawn around a single placeholder species `P00108`. This spec scopes the leap from one placeholder to a populated, biome-coherent ecosystem of ~24 species the player can scan, fight, eat, flee, and (eventually) tame.
...
| Rule | Specifics |
|------|-----------|
| **Six-limb dominance** | Default body plan is six limbs. Variants: 6 legs (hexapods), 4 wings + 2 legs, 2 wings + 4 legs, 6 mixed-use (climber/grasper). 4-limbed forms exist but must be called out as evolutionary outliers. |
| **No giant reptiles** | The "scaly" lineage on Farhaven stayed small and convergently evolved plumage. No T-rex analogues. Largest "reptile-like" creature: dog-sized. |
| **Furry megafauna** | Animals over Large size (>2.5 m) are typically fur-covered, warm-blooded, mammal-analogue. |
...
**A0021 Ash-Coat Pacer.** Mid-size four-legged grazer-omnivore, ash-coloured short fur, called out as the spec's only quadruped outlier. **Lore:** the lineage predates the dominant hexapod radiation — a pre-bottleneck relict that survived in the Rocky / Crash Site margins, where competition from six-limbed grazers was thinnest. Genetically it is its own thing: not a six-limbed creature with limbs reduced, but a separate root that never grew the second pair. The fact that it persists in the same biomes as A0007 and A0008 is itself a story — niche pressure should have erased it long ago. The player meets it as the planet's quiet evidence that the six-limb plan was contested, not preordained.
