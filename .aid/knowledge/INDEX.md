# Knowledge Base Index — Farhaven

Use this index to find the right document before making assumptions.
If your task touches an area covered here, read the relevant document first.

## Active Work

- **work-001-core** — Core MVP (hex grid, movement, gathering, crafting, survival, day/night, building/threats). 8 features, approved.

| Document | Summary |
|----------|---------|
| project-structure.md | Pending |
| external-sources.md | 5 external URLs registered |
| architecture.md | Pending |
| technology-stack.md | Pending |
| module-map.md | Pending |
| coding-standards.md | Pending |
| data-model.md | Pending |
| api-contracts.md | Pending |
| integration-map.md | Pending |
| domain-glossary.md | Pending |
| test-landscape.md | Pending |
| security-model.md | Pending |
| tech-debt.md | Pending |
| infrastructure.md | Pending |
| ui-architecture.md | Pending |
| feature-inventory.md | Pending |
