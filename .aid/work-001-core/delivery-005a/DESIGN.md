...
| 2026-04-09 | Milestones/Events = Recipe kind=EVENT (no new system needed) | Milestones and chapter progression collapse into the Recipe model. A milestone is a Recipe with `kind: EVENT`, `inputs: []`, `outputs: []`, conditions (world_flag, cataloged, etc.) and effects (play_cutscene, journal_entry, set_world_flag, grant_recipe). RecipeRuntime already handles passive recipes. New effect kinds needed: `play_cutscene`, `journal_entry`, `set_world_flag` (which also emits `world_flag_changed` signal). Chapters = cascading EVENT recipes gated by world_flags. Eliminates the need for Chapter/Milestone aggregates from the DDD analysis. Andre's idea — collapses ~30h of planned work into ~4h. |
