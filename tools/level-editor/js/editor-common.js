...
export function renderGearHeader(container, model, options) {
  ...
  const idWrap = _makeLabelInputWrap(idLabel);
  ...
  const nameWrap = _makeLabelInputWrap(nameLabel);
  ...
  const shortWrap = _makeLabelInputWrap(shortLabel);
  ...
  const longWrap = _makeLabelInputWrap(longLabel);
...
/** Build a label+input wrapper (label above the input). */
function _makeLabelInputWrap(labelText) {
