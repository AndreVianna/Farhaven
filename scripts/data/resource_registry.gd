...
func _scan_resources() -> void:
	...
	if dir == null:
		push_error("ResourceRegistry: cannot open %s" % RESOURCES_PATH)
		return
	dir.list_dir_begin()
	var file_name := dir.get_next()
	while file_name != "":
		if file_name.ends_with(".tres"):
			var res := load(RESOURCES_PATH + file_name)
...
## Convenience: get tool speed multiplier for a tool on a resource
